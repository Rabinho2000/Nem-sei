...
| M7 | Alarms, normalised status, operational dashboard (§6, §12) | M2, M6 | Claude (Fatia 1 **LIVE VERIFIED** 2026-08-20: `device_status_facts` imported from V1, migration `0015`, `/diagnostics` UI — see `DIAGNOSTICS.md`. Fatia 2 **LIVE VERIFIED** 2026-08-20: live FusionSolar device read, `getDevList`+`getDevRealKpi`, contract-gated, migration `0016` adds freshness/quality/provenance, canary-proven against the real account (asset 153, two inverters, 5.63 s total V1 downtime, one real defect caught and fixed before persistence, active power/day energy/state all verified, `collectTime` confirmed absent). Fatia 3 **LIVE VERIFIED, then recovered and hardened** 2026-08-20 (same day, later session): persistent, restart-safe, idempotent poll scheduling (`Job`/`ScheduleState`, no new tables), config-gated to one connection, run live for a 70 min/30 min-cadence sustained window — 3/3 cycles succeeded, 6/6 readings, 0 failures, max gap 30:03, 9/15 calls used, confirmed by a later recovery audit to have completed cleanly on schedule, not cut short (`DEVICE_TELEMETRY.md` §9.1). That audit found the real gap: the live cycles were driven by a session-local one-off process, not the standing scheduler container, and there was no hard cap in the code at all. Both fixed: `Scheduler.run_once()` now enqueues `device_status.poll` on its own when configured, gated by a persisted, tamper-evident lifetime cycle cap (§9.2-9.3). A day-plus availability canary is fully prepared (compose override, verified backup, 48-cycle cap reasoned against real account-usage evidence) but **BLOCKED** on one live-database write this session's own safety classifier correctly refused to force through — see `DEVICE_TELEMETRY.md` §10. Density is still not proven sufficient for a full operating day; availability stays off. Fatia 4 **IMPLEMENTED+TESTED** 2026-08-20: a first deterministic operational-findings layer (`diagnostics/findings.py`) — device-unavailable/unknown/no-history/stale, peer power/energy disparity, partial coverage — recomputed per page load, no persisted lifecycle yet, wired into `/diagnostics/assets/<id>`. D1 (planned in `DIAGNOSTICS_PORTFOLIO_TELEGRAM_PLAN.md`) **LIVE VERIFIED** 2026-08-21: persisted `DiagnosticIncident` (migration `0017`) with stable identity/open-resolved lifecycle/duration/provenance, evaluated periodically by the same `Job`/`Scheduler`, zero provider calls needed or made. Live against production: two real 5-minute cycles reconfirmed the same 642 real incidents (opened from V1-imported history) without duplication — `occurrence_count` 1→2, `opened_at` unchanged. Recovery-closes-the-incident proven by the automated suite against a real Postgres, not production (no real recovery to observe in the current static V1-imported data). D2 **IMPLEMENTED, TESTED, deployed** 2026-08-21: `/diagnostics` overview (worst-first, sorted before any display limit) and a new `/diagnostics/incidents` page; a real bug caught by its own search test (missing `Organization` join made every incident search return zero results, fixed). D3 **IMPLEMENTED, TESTED, closed out** 2026-08-21: `NotificationChannel`→`NotificationPolicy`→`NotificationEvent`, structural identity (`incident_id, kind, channel_id`), mock-only Telegram client (no real HTTP client exists in this codebase yet). Two real bugs caught by the required proofs: a concurrency race lost to the unique constraint aborted the whole transaction instead of a graceful no-op (fixed with a SAVEPOINT), and delivery originally shared one transaction for a whole batch, which could have resent an already-delivered message after a mid-batch crash (fixed: one commit per event, proven by a dedicated crash-simulation test). Migration `0018` applied to production; live processing left deliberately OFF. Final dry-run against the complete real universe (644 incidents, `open`+`resolved`): 0 resolved ever, 642/644 (99.7%) over 30 days old (D1 initialization materializing historical V1 import as `opened_at`-dated episodes, not new events), only 2 genuinely recent (the canary's live polling lapsing). Recommended policy reaches zero notifiable today for an honest, quantified reason (rule-code scope, not severity-tuning to hide volume) — a what-if simulation shows including the two structural rule_codes with a 24h escalation would produce 1288 messages in one pass. NO-GO for D4 still, pending explicit human approval of the proposed policy and an inert (channel-disabled) live activation first. D5 **IMPLEMENTED, TESTED, LIVE VERIFIED** 2026-08-21: Portfolio Diagnostics aggregates `diagnostic_incidents` for a portfolio's current resolved membership (`resolve_members`, reused unchanged) -- no new migration, no new finding, one incident on one device counted exactly once per portfolio. Coverage is a real 4-state classification (complete/partial/none/no_devices) specifically so a device-less installation can never read as healthy. A real bug caught by its own filter test (portfolio incident filter options computed from the already-filtered set, the same class of bug D2 had already fixed once, reintroduced here); fixed the same way. Live against both real production portfolios: Solcorelios I (35 installations, 32 with incidents, 0 healthy, 186 warnings, all backlog >39 days old) and Solcorelios II (22 installations, 13 with incidents, 0 healthy, 48 warnings, all backlog) -- zero recent incidents in either, because the only recent incidents system-wide belong to the canary asset, which is not a member of either real portfolio. No migration/normalization proposed for the historical incidents: the data is honest, not corrupted, and the recent/historical split already correctly represents it. D6 **IMPLEMENTED, TESTED** 2026-08-21: periodic digest (`DigestRun`, migration `0019`), immediate alerts (D3) and periodic digest kept explicitly separate. Windows chain (each digest's window_start is literally the previous digest's window_end, bootstrapped to one interval for the first run) so "since the last digest" is always true, not approximated. A real bug in the window-chaining logic caught by the idempotency test itself: requesting the same window_end twice found the just-created digest as its own "previous" and computed an empty window, returning None instead of the existing row -- fixed by checking window_end-based idempotency before any chaining logic runs. New/persistent/resolved classified directly from opened_at/resolved_at, capped at 3 named installations per portfolio so the digest never lists hundreds of incidents. Delivery reuses D3's exact restart-safe discipline (one transaction per attempt, never falsely marked delivered). Live dry-run (read-only, zero digest_runs written to production) against both real portfolios: "Sem alterações desde o último digest" for the last 24h (0 new, 0 resolved, same 234 backlog warnings as D5). Recommendation: still not D4 -- the same policy-approval decision identified in D3/D5, now confirmed from a third independent angle. Sigenergy has no device-level contract to build on, evidenced not assumed — see `DEVICE_TELEMETRY.md`). Policy review **2026-08-23** (`DIAGNOSTICS_PORTFOLIO_TELEGRAM_PLAN.md` §28): the user's 6 concrete notification decisions translated into two proposed `NotificationPolicy` configurations (critical-immediate; warning-escalation at 24h for the disparity rule_codes) plus a documented "no policy" digest-only bucket for the 4 structural rule_codes -- no policy persisted. Formalising the decisions caught two real bugs in `_decide_for_policy`: `baseline_at` was never checked in the escalation branch (would have fired "escalated" for the entire backlog on first activation), and `notify_on_resolve` fired regardless of whether the incident had ever actually alerted (fixed: now requires a prior `opened`/`escalated` event). Both fixed, 4 tests added/rewritten, full suite 605 passed/1 skipped. Live read-only dry-run against the real 644 open incidents: 0 critical, 0 disparity-rule incidents exist today, so this policy would generate **zero Telegram messages today** -- all 644 correctly route to the existing D6 digest untouched. One gap documented, not implemented: a backlog incident that worsens in severity after activation is not distinguished from one that never changed (`baseline_at` only compares `opened_at`), left open pending a real case to design against (zero practical impact today). No `NotificationPolicy`/`NotificationChannel` row created, no Telegram sent). Baseline gap closed **2026-08-23** (`DIAGNOSTICS_PORTFOLIO_TELEGRAM_PLAN.md` §29): `baseline_at` alone could only compare `opened_at`, so a pre-existing incident that later worsened into scope was indistinguishable from one that never changed -- both permanently excluded. Fixed with a new table, `notification_baseline_snapshots` (migration `0020`), freezing an incident's severity the first time a policy evaluates it at/after baseline; a later evaluation compares current scope against that frozen snapshot instead. A real design flaw was caught before the test suite: capturing the snapshot only once already in scope is too late (the current value already reflects the transition), fixed with a separate pass run before any severity filtering. 6 explicit scenario tests plus an 8-thread concurrency test added; full suite 611 passed/1 skipped (also caught and fixed a migration revision-id-too-long-for-`alembic_version` deployment bug along the way). Read-only dry-run against the real 644 incidents (rolled back, zero writes) confirmed 0 events under the new logic. The two approved `NotificationPolicy` rows were then created for real in production, bound to a `NotificationChannel` with `enabled=False` (structurally zero external calls regardless of policy decisions), `baseline_at` = real activation moment. One real `evaluate_and_process_notifications` pass run against production: 0 events, 644 baseline snapshots captured (all under the critical-immediate policy, all severity=warning, none transitioned). No Telegram sent, D4 still not started |
