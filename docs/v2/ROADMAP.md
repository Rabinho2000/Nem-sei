...
| M7 | Alarms, normalised status, operational dashboard (§6, §12) | M2, M6 | Claude (Fatia 1 done 2026-08-20: `device_status_facts` imported from V1, migration `0015`, `/diagnostics` UI — see `DIAGNOSTICS.md`. Fatia 2 done 2026-08-20: live FusionSolar device read, `getDevList`+`getDevRealKpi`, contract-gated, migration `0016` adds freshness/quality/provenance, **canary-proven live** against the real account (asset 153, two inverters, 5.63 s total V1 downtime, one real defect caught and fixed before persistence, active power/day energy/state all verified, `collectTime` confirmed absent). Fatia 3 done 2026-08-20: persistent, restart-safe, idempotent poll scheduling (`Job`/`ScheduleState`, no new tables), config-gated to one connection, run live for a 70 min/30 min-cadence sustained window — 3/3 cycles succeeded, 6/6 readings, 0 failures, max gap 30:03, 9/15 calls used. Density is strongly extrapolated but not yet proven sufficient for a full operating day; availability stays off. Sigenergy has no device-level contract to build on, evidenced not assumed — see `DEVICE_TELEMETRY.md`) |
