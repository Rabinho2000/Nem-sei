...
class Asset(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_assets_installation", "installation_id"),
    )
    ...
    # Present since 0001, always NULL in practice (no importer and no UI has
    # ever written to them) and now superseded by `Installation.latitude` /
    # `.longitude`, which carry the provenance these two never could. Kept,
    # unwritten, for backward compatibility -- see `installations/models.py`.
    latitude: Mapped[Decimal | None] = mapped_column(Numeric(10, 7))
    longitude: Mapped[Decimal | None] = mapped_column(Numeric(10, 7))
    technical_notes: Mapped[str | None] = mapped_column(Text)
    # The physical/operational site this canonical central belongs to. See
    # `installations/models.py` for why the two are separate entities.
    # Nullable during the transition: 0031 backfills one Installation per
    # existing Asset, but the column itself does not require it.
    installation_id: Mapped[int | None] = mapped_column(ForeignKey("installations.id", ondelete="SET NULL"))
