"""Reporting periods, ported from V1's `nemsei.reporting.periods`.

A period decides which months a report covers and how it is labelled to the
customer, so it is copied rather than reinterpreted: a report titled
"T3 2026" must cover exactly the months V1 covered under that title. The module
imports only the standard library and V2's own rule types.

One thing does not carry across unchanged. V1's `ReportingPeriod.end` is the
**last day of the period**, inclusive, while V2's persistence layer
(`ReportingDataset.period_end`, `datasets.month_end`) uses an exclusive end.
Both conventions are kept where they belong and `exclusive_end` converts between
them, because silently mixing an inclusive and an exclusive bound is how a
report loses or gains a day.
"""
...
from nemsei.reporting.rules.types import ReportPeriodType, ReportingPeriod
...
def exclusive_end(period: ReportingPeriod) -> date:
    """V1's inclusive last day as the exclusive bound V2's persistence uses."""
