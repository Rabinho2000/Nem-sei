"""V1-parity parser for customer financial model workbooks.

Ported verbatim from the frozen V1 module `monitoring_board.reporting
.financial_models` so that V2 reproduces V1's numbers exactly, cell for cell.
V2 does not import V1; this is a copy under V2 ownership, and any change to it
must be justified against a real workbook rather than against taste.

Only the `financial_automatic_as_sold` family has a real workbook on the server
to validate against; see `docs/v2/FINANCIAL_MODEL_SOURCES.md`. The remaining
families are carried over unchanged and remain unvalidated against real files.

The parser reads saved values only: it never executes macros and never
recalculates formulas, and it never turns a missing value into a zero.
"""
