"""Customer report PDF, ported from V1's `nemsei.reporting.customer_pdf`.

V1 draws this document on a reportlab canvas and it is what customers actually
receive, so it is copied rather than reinterpreted. Its only V1 dependencies
were the billing rules and their dataclasses, both already under V2 ownership,
and an optional logo path supplied by the caller. It reads no database and calls
no provider.
"""
...
from nemsei.reporting.rules.billing import (
...
from nemsei.reporting.rules.types import BillingConfig, BillingEnergyBase, BillingMode, EnergyBreakdown, ReportType
