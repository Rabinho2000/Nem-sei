...
def main() -> int:
    ...
    # Every compose file the deployment carries, not just the base one: an
    # overlay that mounted V1's storage would have slipped past a check that
    # only ever read docker-compose.v2.yml.
    parser.add_argument("--compose-file", type=Path, nargs="+", default=[Path("docker-compose.v2.yml")])
    ...
    for compose_file in args.compose_file:
        ...
        if "Nem-sei/data" in compose_text or "monitoring_board.db" in compose_text:
            print(f"V2 Compose must not reference V1 runtime data: {compose_file}", file=sys.stderr)
            return 1
