...
def upgrade() -> None:
    ...
    op.create_index("uq_organizations_normalized_tax_id", "organizations", ["normalized_tax_id"], unique=True, sqlite_where=sa.text("normalized_tax_id IS NOT NULL AND normalized_tax_id != ''"))
    ...
    op.create_index("uq_asset_provider_mappings_active_external", "asset_provider_mappings", ["provider_connection_id", "resource_kind", "normalized_external_id"], unique=True, sqlite_where=sa.text("mapping_status = 'active' AND valid_to IS NULL"))
